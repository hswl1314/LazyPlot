...
draw_oplsda <- function(df, var_names = NULL, Group = NULL, group_labels = NULL,
  ...
  # Prepare data matrix for OPLS-DA
  ...
  plot_data <- data.frame(
    t1 = as.vector(oplsda_model@scoreMN),
    to1 = as.vector(oplsda_model@orthoScoreMN),
    Group = Y
  )
...
#' @title Create Example Data for OPLS-DA Analysis
#' @name create_example_data
#' @description Creates example data with two groups for demonstrating OPLS-DA analysis
#'
#' @param n_samples Number of samples to generate (default: 30)
...
create_example_data <- function(n_samples = 30) {
  ...
  group1 <- data.frame(
    SampleID = paste0("Sample_A", 1:(n_samples/2)),
    Group = "Control",
    Variable1 = rnorm(n_samples/2, mean = 5, sd = 1),
    Variable2 = rnorm(n_samples/2, mean = 10, sd = 2),
    Variable3 = rnorm(n_samples/2, mean = 15, sd = 1.5)
  )
  
  group2 <- data.frame(
    SampleID = paste0("Sample_B", 1:(n_samples/2)),
    Group = "Treatment",
    Variable1 = rnorm(n_samples/2, mean = 7, sd = 1),
    Variable2 = rnorm(n_samples/2, mean = 13, sd = 2),
    Variable3 = rnorm(n_samples/2, mean = 18, sd = 1.5)
  )
  
  # Combine data
  example_data <- rbind(group1, group2)
  return(example_data)
}
